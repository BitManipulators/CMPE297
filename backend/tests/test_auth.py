...
class TestRAGEndpoints:
    ...
    def test_rag_status_available(self, client, mocker):
        ...
        mock_rag.index_name = "test-index"
        mock_rag.plant_cache = {"plant1": {}, "plant2": {}}
        
        mocker.patch("main.RAG_AVAILABLE", True)
        mocker.patch("main.rag_service", mock_rag)
    ...
    def test_rag_test_with_services(self, client, mocker):
        ...
        mock_rag = Mock()
        mock_rag.bedrock_runtime = Mock()
        mock_rag.embedding_model = "cohere.embed-english-v3"
        mock_rag._generate_embedding.return_value = [0.1] * 1024
        ...
        assert data["pinecone"]["available"] is True
    ...
    def test_index_plants_file_not_found(self, client, mocker):
        """Test indexing when plant data file not found"""
        mock_rag = Mock()
        mock_rag.is_available.return_value = True
        ...
        # Server returns 500 when indexing fails (file not found causes error)
        assert response.status_code == 500
    ...
    def test_index_plants_success(self, client, mocker, tmp_path, sample_plant_data):
        ...
        mock_rag = Mock()
        mock_rag.is_available.return_value = True
        mock_rag.load_and_index_plants.return_value = True
        mock_rag.plant_cache = {"plant1": {}}
